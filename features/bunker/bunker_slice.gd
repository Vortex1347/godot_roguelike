...
func _ready() -> void:
	...
	if bunker_state != null:
		_player.global_position = bunker_state.get("player_position")

	_player.movement_enabled = not _inventory_shell.is_open()
	_refresh_prompt()
	_show_status("Bunker slice ready. Stash transfers are live.")
...
func _activate_hotspot(hotspot: Area2D) -> void:
	...
	match action_id:
		...
		BUNKER_ACTIONS.OPEN_TRADER:
			_show_status("Trader flow reserved for the next slice.")
		BUNKER_ACTIONS.OPEN_TASK_BOARD:
			_show_status("Task board is a future hotspot in this milestone.")
		_:
