...
## Current Pass: 3D Bunker Benchmark

### Direction reset

- `ZERO Sievert` больше не является `1:1` visual target. После pivot он используется как anchor по extraction mood, utility density и bunker readability.
- Активный контракт теперь: `3D`, `tilted orthographic`, `gritty low-poly`, `inspired-only`.

### Bunker composition

- Official Steam overview: [ZERO Sievert on Steam](https://store.steampowered.com/app/1782120/ZERO_Sievert/)
  Transfer: bunker читается как safe operations hub с utility stations по периметру и clear path через центр. В текущем `3D` benchmark это переведено в perimeter stations + центральный коридор.
- Official Steam poster: [tutorial / bunker-related poster](https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/1782120/extras/f754439d67ad8e548ef0c8de98761f83.poster.avif?t=1772458263)
  Transfer: плотная, но не хаотичная компоновка зон; у нас это выведено в stash upper-right, trader lower-right, task board upper-left, service corner lower-left.
- Public gameplay guide: [This Is The ZERO Sievert Bunker! - The Guide | ZERO Sievert](https://www.youtube.com/watch?v=vVI-Vbpcm4k)
  Transfer: подтверждает utility-heavy bunker и иерархию станций; в `3D` их нужно сохранять через форму, свет и проходы, а не через pixel-art detail.
...
- Official Steam poster: [store media poster 01](https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/1782120/extras/2b3e4d3636d14c30c6bdbe09c29d0074.poster.avif?t=1772458263)
  Transfer: small readable top-down hunter silhouette с отдельным оружейным направлением. В `3D` benchmark это переводится в boxy hunter body, distinct backpack mass и передний weapon nub.
...
- Official Steam overview text on bunker/traders/modding stations: [ZERO Sievert on Steam](https://store.steampowered.com/app/1782120/ZERO_Sievert/)
  Transfer: bunker должен ощущаться как chain of useful stations, а не пустой лобби-зал. Поэтому активный `3D` blockout уже держит stash, trader counter, task board и service corner.
- Public gameplay guide: [This Is The ZERO Sievert Bunker! - The Guide | ZERO Sievert](https://www.youtube.com/watch?v=vVI-Vbpcm4k)
...
- Official Steam poster: [wasteland lighting poster](https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/1782120/extras/4c36fff43cd0af965ee17bd4625e178f.poster.avif?t=1772458263)
  Transfer: даже в bunker indoor pass важна направленная атмосферность и light pooling; поэтому текущий `3D` benchmark получил fog + отдельные station lights, а не плоский общий залив.

### Inventory / stash proportions
...
- Public gameplay guide: [This Is The ZERO Sievert Bunker! - The Guide | ZERO Sievert](https://www.youtube.com/watch?v=vVI-Vbpcm4k)
  Transfer: текущий shell берет за рабочий контракт `12x6` для stash и `8x6` для player inventory, с одновременной видимостью обоих контейнеров.

### Trader screen layout
...
- Official Steam poster: [loot and questing poster](https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/1782120/extras/2f83efa1664e81bfb6c202ba284b0d74.poster.avif?t=1772458263)
  Transfer: trader flow пока не реализуется, но торговая зона уже присутствует как hotspot-reserved `3D` prop cluster, чтобы композиция bunker не потребовала переезда в следующем milestone.

### Proxy asset policy

- Active repo contract: любые replace-later модели идут через `assets/proxy/` и реестр `assets/proxy/PROXY_asset_registry.csv`.
- Текущий bunker benchmark намеренно использует `internal proxy blockout`, чтобы 3D-моделлер потом мог заменить wrapper-сцены без переделки feature flow.
