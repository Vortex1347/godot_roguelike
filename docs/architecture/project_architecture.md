...
- `runtime/models/BunkerSessionState` теперь хранит обе формы bunker position state:
  ...
  - `player_position_3d` для активного `3D` bunker benchmark
...
- Активный startup flow теперь использует `3D bunker` scene; legacy `2D bunker` остается в репозитории как short-term reference-only код до cleanup-задачи.
